"""
Naver Maps API를 사용한 Geocoding (SQLite 캐시 버전)
- 원본 쿼리 주소를 키로 사용
- 지번/도로명 주소 모두 저장
"""
...
# 캐시 설정
CACHE_DB = RESULTS_DIR / "geocoding_cache_naver.db"
...
class GeocodingCacheSQLite:
    """SQLite 기반 Geocoding 캐시 (싱글톤 패턴)"""

    _instance: "GeocodingCacheSQLite | None" = None
    _initialized: bool = False

    def __new__(cls, db_file: Path = CACHE_DB) -> "GeocodingCacheSQLite":
        if cls._instance is None:
            cls._instance = super().__new__(cls)
        return cls._instance

    def __init__(self, db_file: Path = CACHE_DB):
        if GeocodingCacheSQLite._initialized:
            return

        self.db_file = db_file
        self._ensure_table()
        GeocodingCacheSQLite._initialized = True
        logger.info("Naver SQLite 캐시 연결: %s", self.db_file)
    ...
    def get(self, query_address: str) -> dict[str, Any] | None:
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            if result:
                jibun_address, road_address, lat, lon, status, error, cached_at = result
                if status == "success":
                    return {
                        "status": "success",
...
def _get_cache() -> GeocodingCacheSQLite:
    """캐시 인스턴스 반환"""
    global _cache
    if _cache is None:
        _cache = GeocodingCacheSQLite()
    return _cache
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt > 0:
                ...
                logger.info(
                    "재시도 %d/%d: %d초 대기 중...", attempt, max_retries - 1, wait_time
                )
                time.sleep(wait_time)

            response = requests.get(
                NAVER_GEOCODING_URL, headers=headers, params=params, timeout=API_TIMEOUT
            )
            ...
            if data.get("status") == "OK" and data.get("addresses"):
                ...
                logger.debug(
                    "API 성공: %s → %s", query_address, jibun_address or road_address
                )
                return (lat, lon)
            logger.warning(
                "Naver: 검색 결과 없음 - %s (전처리: %s)",
                query_address,
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    "Naver API 연결 오류 (시도 %d/%d): %s", attempt + 1, max_retries, e
                )
                continue
            logger.error("Naver API 연결 실패 (모든 재시도 실패): %s", e)
            if use_cache:
                cache.set(
                    query_address,
                    {
            ...
            logger.error("Naver API HTTP 오류: %s", e)
            if use_cache:
