#!/usr/bin/env python3
"""
main58_analyze_remaining_life.py

D_final_org 값의 범위별 분포 분석 및 잔여 수명 평가
...
출력:
  - results/main58_analyze_remaining_life/d_final_distribution.png
  - results/main58_analyze_remaining_life/remaining_life_by_risk.png
  - results/main58_analyze_remaining_life/risk_category_summary.csv
  - results/main58_analyze_remaining_life/critical_pipes.csv
...
class RiskThresholds:
    ...
    # D_final_org 기준 (분포 최적화를 위한 사용자 정의)
    critical: float = 0.400       # 위험
    monitor: float = 0.150        # 감시
    warning: float = 0.030        # 주의
    safe: float = 0.010           # 안전
    very_safe: float = 0.010      # 매우 안전
    ...
    # 색상 정의
    color_immediate: str = '#FF0000'     # 빨강
    color_very_critical: str = '#FF6347' # 토마토(빨강-주황)
    color_critical: str = '#FFD700'      # 골드(노랑)
    color_warning: str = '#9ACD32'       # 연두
    color_safe: str = '#2E8B57'          # 초록
...
class RemainingLifeAnalyzer:
    ...
    def analyze_d_final_distribution(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for region in regions:
            for pipe_type in ['PIPE_LM', 'SPLY_LS']:
                ...
                if len(region_df) > 0 and 'D_final_org' in df.columns:
                    values = region_df['D_final_org'].dropna()
                    if len(values) > 0:
    ...
    def categorize_risk(self, df: pd.DataFrame, region: str = None) -> pd.DataFrame:
        ...
        if 'D_final_org' not in df_region.columns:
            return pd.DataFrame()

        df_region = df_region.dropna(subset=['D_final_org'])

        # D_final 기반 위험도 분류 (5단계)
        df_region['risk_category'] = pd.cut(
            df_region['D_final_org'],
    ...
    def plot_distribution_charts(self, df: pd.DataFrame):
        """D_final_org 분포 차트 생성"""
        print("\n3. 분포 차트 생성")

        # 필요한 컬럼만 선택하여 plot_df 생성
        plot_df = df[['zone', 'DATA_SRC', 'D_final_org']].copy()
        plot_df = plot_df.rename(columns={'zone': 'Region', 'DATA_SRC': 'Type'})
        plot_df = plot_df.dropna(subset=['D_final_org'])

        # Figure 1: 히스토그램과 박스플롯
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('D_final_org 값 분포 분석', fontsize=16, fontweight='bold')

        # 1-1: 전체 히스토그램 (로그 스케일)
        ax1 = axes[0, 0]
        ax1.hist(plot_df['D_final_org'], bins=50, edgecolor='black', alpha=0.7)
        ax1.set_xlabel('D_final_org')
        ax1.set_ylabel('빈도')
        ax1.set_title('전체 D_final_org 분포')
        ax1.set_yscale('log')
        ax1.grid(True, alpha=0.3)

        # 위험도 임계값 표시 (5단계)
        ax1.axvline(self.thresholds.very_safe, color=self.thresholds.color_safe, linestyle='--',
                   label=f'매우 안전 (<{self.thresholds.very_safe:.3f})')
        ax1.axvline(self.thresholds.safe, color=self.thresholds.color_warning, linestyle='--',
                   label=f'안전 ({self.thresholds.safe:.3f})')
        ax1.axvline(self.thresholds.warning, color=self.thresholds.color_critical, linestyle='--',
                   label=f'주의 ({self.thresholds.warning:.3f})')
        ax1.axvline(self.thresholds.monitor, color=self.thresholds.color_very_critical, linestyle='--',
                   label=f'감시 ({self.thresholds.monitor:.3f})')
        ax1.axvline(self.thresholds.critical, color=self.thresholds.color_immediate, linestyle='--',
                   label=f'위험 (>{self.thresholds.critical:.3f})')
        ax1.legend(loc='upper right', fontsize=8)

        # 1-2: 지역별 박스플롯
        ax2 = axes[0, 1]
        plot_df.boxplot(column='D_final_org', by='Region', ax=ax2)
        ax2.set_xlabel('지역')
        ax2.set_ylabel('D_final_org')
        ax2.set_title('지역별 D_final_org 분포')
        ax2.set_yscale('log')
        plt.sca(ax2)
        ...
        # 1-3: 타입별 비교
        ax3 = axes[1, 0]
        plot_df.boxplot(column='D_final_org', by='Type', ax=ax3)
        ax3.set_xlabel('파이프 타입')
        ax3.set_ylabel('D_final_org')
        ax3.set_title('파이프 타입별 D_final_org 분포')
        ax3.set_yscale('log')
        ...
        counts = [
            (plot_df['D_final_org'] < self.thresholds.very_safe).sum(),  # 매우 안전: < 0.010
            ((plot_df['D_final_org'] >= self.thresholds.very_safe) & (plot_df['D_final_org'] < self.thresholds.safe)).sum(),  # 안전: 0.010 ~ 0.010
            ((plot_df['D_final_org'] >= self.thresholds.safe) & (plot_df['D_final_org'] < self.thresholds.warning)).sum(),  # 주의: 0.010 ~ 0.030
            ((plot_df['D_final_org'] >= self.thresholds.warning) & (plot_df['D_final_org'] < self.thresholds.monitor)).sum(),  # 감시: 0.030 ~ 0.150
            (plot_df['D_final_org'] >= self.thresholds.critical).sum()  # 위험: >= 0.400
        ]
        ...
        # 5단계 색상 설정
        colors_5levels = [
            self.thresholds.color_safe,           # 초록 - 매우 안전
            self.thresholds.color_warning,         # 연두 - 안전
            self.thresholds.color_critical,        # 노랑 - 주의
            self.thresholds.color_very_critical,   # 주황 - 감시
            self.thresholds.color_immediate        # 빨강 - 위험
        ]
        ...
        fig.suptitle('지역별 D_final_org 상세 분포', fontsize=16, fontweight='bold')
        ...
        for idx, region in enumerate(regions):
            ...
            if not region_df.empty and 'D_final_org' in region_df.columns:
                # 타입별 데이터 분리
                pipe_vals = region_df[region_df['DATA_SRC'] == 'PIPE_LM']['D_final_org'].dropna()
                sply_vals = region_df[region_df['DATA_SRC'] == 'SPLY_LS']['D_final_org'].dropna()
                ...
                # 통계 정보 추가
                all_vals = region_df['D_final_org'].dropna()
                if len(all_vals) > 0:
    ...
    def plot_risk_analysis(self, df: pd.DataFrame):
        ...
        # 2-1: 위험도 카테고리별 파이 차트
        ax1 = axes[0, 0]
        colors = [self.thresholds.color_safe, self.thresholds.color_warning,
                  self.thresholds.color_critical, self.thresholds.color_very_critical,
                  self.thresholds.color_immediate]
        ...
        for region in regions:
            ...
            region_df = df[df['zone'] == region]
            if 'D_final_org' in region_df.columns:
                values = region_df['D_final_org'].dropna()
                for v in values:
                    if v < self.thresholds.very_safe:
        ...
        for pipe_type in ['PIPE_LM', 'SPLY_LS']:
            ...
            if 'D_final_org' in type_df.columns:
                values = type_df['D_final_org'].dropna()
                for v in values:
                    if v < self.thresholds.very_safe:
    ...
    def save_critical_pipes(self, df: pd.DataFrame):
        ...
        if 'D_final_org' not in df.columns:
            print("  - D_final_org 컬럼이 없습니다.")
            return pd.DataFrame()

        # 감시 임계값 이상 파이프 필터링 (감시, 위험)
        critical_mask = df['D_final_org'] >= self.thresholds.monitor

        if critical_mask.any():
            critical_df = df[critical_mask].copy()

            # 필요한 컬럼만 선택
            cols_to_keep = ['FTR_IDN', 'zone', 'DATA_SRC', 'D_final_org']
            ...
            critical_df = critical_df.sort_values('D_final_org', ascending=False)
    ...
    def generate_report(self, distribution_df: pd.DataFrame, critical_df: pd.DataFrame):
        ...
        with open(report_path, 'w') as f:
            f.write("# D_final_org 분포 및 잔여 수명 분석 보고서\n\n")
            ...
            if not critical_df.empty:
                ...
                f.write("\n### 상위 10개 감시/위험 파이프\n")
                f.write("| 순위 | FTR_IDN | 지역 | 타입 | D_final_org |\n")
                f.write("|------|---------|------|------|------------:|\n")

                for idx, row in critical_df.head(10).iterrows():
                    f.write(f"| {idx+1} | {row['FTR_IDN']} | {row['Region']} | "
                           f"{row['Type']} | {row['D_final_org']:.6f} |\n")
            else:
                f.write("위험 임계값을 초과하는 파이프가 없습니다.\n")

            f.write("\n## 4. 권장사항\n\n")
            f.write("1. **긴급 점검 필요**: D_final_org > 0.024인 파이프\n")
            f.write("2. **정기 모니터링**: D_final_org 0.010-0.024 범위의 파이프\n")
...
def main():
    """메인 실행 함수"""
    print("=" * 60)
    print("main58_analyze_remaining_life.py - D_final 분포 및 잔여 수명 분석")
    print("=" * 60)

    try:
        # 분석기 초기화
        output_dir = Path("results/main58_analyze_remaining_life")
