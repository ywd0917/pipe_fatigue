...
의존성 없는 데이터:
  ...
  - 교통 정보 없음 → K_traffic=1.0 (traffic CSV 없을 경우 기본값)
...
from fatigue_db_writer import write_0200_to_db
...
# 임시 CSV 저장 경로
TEMP_PIPE_LM_CSV = OUTPUT_DIR / "0200_PIPE_LM_temp.csv"
TEMP_SPLY_LS_CSV = OUTPUT_DIR / "0200_SPLY_LS_temp.csv"
...
def apply_zone_and_rename(df: pd.DataFrame, pipe_type: str) -> pd.DataFrame:
    """zone=0200 설정 후 0200_ prefix 제거"""
    df = df.copy()
...
def main() -> None:
    ...
    output_file = OUTPUT_DIR / "0200_fatigue_merged_zone_fixed.csv"
    merged.to_csv(output_file, index=False, encoding="utf-8-sig")

    write_0200_to_db(merged)
